...
interpreter_obj interpreter_print(type_vm *tp) {
    int n = 0;
    interpreter_obj e;
    interpreter_LOOP(e)
        if (n) { printf(" "); }
...
interpreter_obj interpreter_bind(type_vm *tp) {
...
interpreter_obj interpreter_min(type_vm *tp) {
...
interpreter_obj interpreter_max(type_vm *tp) {
    interpreter_obj r = interpreter_OBJ();
    interpreter_obj e;
    interpreter_LOOP(e)
        if (interpreter_cmp(tp,r,e) < 0) { r = e; }
...
interpreter_obj interpreter_copy(type_vm *tp) {
    interpreter_obj r = interpreter_OBJ();
    int type = r.type;
...
interpreter_obj interpreter_float(type_vm *tp) {
    interpreter_obj v = interpreter_OBJ();
...
interpreter_obj interpreter_save(type_vm *tp) {
...
interpreter_obj interpreter_load(type_vm *tp) {
    FILE *f;
    long l;
    interpreter_obj r;
    char *s;
...
interpreter_obj interpreter_fpack(type_vm *tp) {
    type_vmNum v = type_vmNum();
    interpreter_obj r = interpreter_string_t(tp,sizeof(type_vmNum));
    *(type_vmNum*)r.string.val = v;
    return interpreter_track(tp,r);
}

interpreter_obj interpreter_abs(type_vm *tp) {
    return interpreter_number(fabs(interpreter_float(tp).number.val));
}
interpreter_obj interpreter_int(type_vm *tp) {
...
interpreter_obj interpreter_round(type_vm *tp) {
    return interpreter_number(_roundf(interpreter_float(tp).number.val));
}

interpreter_obj interpreter_exists(type_vm *tp) {
...
interpreter_obj interpreter_mtime(type_vm *tp) {
    char fname[interpreter_CSTR_LEN]; interpreter_cstr(tp,interpreter_STR(),fname,interpreter_CSTR_LEN);
    struct stat stbuf;
...
int _interpreter_lookup_(type_vm *tp,interpreter_obj self, interpreter_obj k, interpreter_obj *meta, int depth) {
...
int _interpreter_lookup(type_vm *tp,interpreter_obj self, interpreter_obj k, interpreter_obj *meta) {
 ...
 * None
 */
interpreter_obj interpreter_setmeta(type_vm *tp) {
...
interpreter_obj interpreter_getmeta(type_vm *tp) {
    interpreter_obj self = interpreter_TYPE(interpreter_DICT);
 ...
 */
interpreter_obj interpreter_object(type_vm *tp) {
    interpreter_obj self = interpreter_dict(tp);
    self.dict.dtype = 2;
    return self;
}

interpreter_obj interpreter_object_new(type_vm *tp) {
...
interpreter_obj interpreter_object_call(type_vm *tp) {
 ...
 * dict.
 */
interpreter_obj interpreter_getraw(type_vm *tp) {
    interpreter_obj self = interpreter_TYPE(interpreter_DICT);
    self.dict.dtype = 0;
    return self;
}
 ...
 */
interpreter_obj interpreter_class(type_vm *tp) {
 ...
 */
interpreter_obj interpreter_builtins_bool(type_vm *tp) {
    interpreter_obj v = interpreter_OBJ();
